  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow border border-gray-200 hover:shadow-md hover:border-[#FBBF24] transition-all duration-300 text-sm">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-base font-bold text-[#1F2937]">{lawyer.user.name}</h3>
          <p className="text-xs text-gray-500">{lawyer.experience_years || '0'} yrs experience</p>
        </div>
      </div>

      <div className="mt-3">
        <h4 className="font-medium text-gray-600 text-xs mb-1">Specializations:</h4>
        <div className="flex flex-wrap gap-1">
          {lawyer.specializations.map(spec => (
            <span key={spec} className="text-[10px] font-medium bg-gray-100 text-gray-700 py-0.5 px-2 rounded-full">
              {spec}
            </span>
          ))}
        </div>
...
const FindLawyerPage = () => {
  ...
  <div className="flex bg-[#F9FAFB] min-h-screen">
    <Sidebar />
    <div className="ml-64 p-6 flex-1 flex flex-col"> {/* ↓ Reduced padding */}
      <Header />
      <main className="flex-1 p-4 overflow-y-auto">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6"> {/* ↓ Gap reduced */}
            {lawyers.map((lawyer) => (
              <LawyerCard
                key={lawyer.user_id}
                lawyer={lawyer}
        ...
        {!isLoading && lawyers.length === 0 && (
          <div className="text-center py-12 bg-white rounded-lg border mt-6">
            <h3 className="text-lg font-semibold text-gray-700">No Lawyers Found</h3>
