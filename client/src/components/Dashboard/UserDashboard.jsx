...
const recentCases = [
  {
    id: 9,
    title: "Cheating and Fraud in Online Purchase",
    status: "Lawyer Engaged",
    counsel: "Arjun",
  },
  {
    id: 8,
...
const notifications = [
  {
    id: 1,
    message: "Lawyer Arjun has accepted your case.",
    time: "2 hours ago",
    link: "/case/9",
  },
  {
    id: 2,
    message: "A new document 'FIR Draft' was generated.",
    time: "1 day ago",
    link: "/case/9",
  },
];
        ...
        <main className="flex-1 p-8 overflow-hidden">
          ...
          >
            ...
            <p className="mt-1 text-gray-500 max-w-2xl">
              From filing your first report to tracking case progress, LexiVerse
            ...
            >
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                  <Link
                    to="/chatbot"
                    className="block p-6 bg-orange-50 rounded-lg border border-yellow-200 hover:shadow-md hover:border-yellow-300 transition-all"
                  >
                    {/* --- Color Changed Here --- */}
                ...
                <div className="space-y-3">
                  {recentCases.map((c) => (
                    <Link
                      to={`/case/${c.id}`}
                      key={c.id}
                      className="block p-4 rounded-lg hover:bg-gray-50 border transition-all"
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          {c.counsel ? (
                            <p className="text-xs text-green-600 font-medium flex items-center gap-2 mt-1">
                              <FaUserTie /> Engaged: {c.counsel}
                            </p>
                          ) : (
                            <p className="text-xs text-gray-500 mt-1 flex items-center gap-2">
                              <FaHourglassHalf /> Awaiting lawyer assignment
                            </p>
                          )}
                        </div>
                        <span
                          className={`text-xs font-bold py-1 px-3 rounded-full ${c.status === "Resolved" ? "bg-gray-200 text-gray-800" : c.status === "Lawyer Engaged" ? "bg-green-100 text-green-800" : "bg-blue-100 text-blue-800"}`}
                        >
                          {c.status}
                        </span>
                      </div>
                ...
                <ul className="space-y-4">
                  {notifications.map((n) => (
                    <li key={n.id} className="flex gap-4">
                      <div className="bg-blue-100 text-blue-600 rounded-full h-8 w-8 flex-shrink-0 flex items-center justify-center">
                        <FaBell />
                      </div>
                      <div>
