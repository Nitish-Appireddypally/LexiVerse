...
const Step2OffenseDetails = ({ data = {}, update, onNext, onBack }) => {
  ...
  const handleChange = (e) => {
    const updated = { ...details, [e.target.name]: e.target.value };
    setDetails(updated);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Time of Offense (approx.)
          </label>
          <input
            type="time"
            name="offenseTime"
            value={details.offenseTime}
            onChange={handleChange}
            className="w-full p-2 border border-gray-300 rounded-md shadow-sm"
          />
        </div>
      ...
      <div>
        ...
        <textarea
          name="placeOfOffense"
          value={details.placeOfOffense}
          onChange={handleChange}
          rows="3"
          className="w-full p-2 border border-gray-300 rounded-md shadow-sm"
          placeholder="Full address where the incident occurred, including city and state."
        />
      </div>
